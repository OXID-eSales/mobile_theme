...
<form class="js-oxValidate" action="[{ $oViewConf->getSslSelfLink() }]" method="post">
    ...
    <ul class="form clear">
        ...
        <li [{if $aErrors && !$oView->getContactSubject()}]class="oxInValid"[{/if}]>
            <input type="text" name="c_subject" placeholder="[{ oxmultilang ident="SUBJECT" suffix="COLON" }]" size="70" maxlength=80 value="[{$oView->getContactSubject()}]" class="js-oxValidate js-oxValidate_notEmpty">
                    <p class="oxValidateError">
                <span class="js-oxError_notEmpty">[{ oxmultilang ident="ERROR_MESSAGE_INPUT_NOTALLFIELDS" }]</span>
            </p>
        </li>
