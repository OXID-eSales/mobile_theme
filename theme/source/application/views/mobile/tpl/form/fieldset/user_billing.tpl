...
[{/if}]
    ...
    <li [{if $aErrors.oxuser__oxfax}]class="invalid-field"[{/if}]>
        <input [{if $oView->isFieldRequired(oxuser__oxfax) }] class="js-oxValidate js-oxValidate_notEmpty" [{/if}]placeholder="[{ oxmultilang ident="FORM_FIELDSET_USER_BILLING_FAX" }][{if $oView->isFieldRequired(oxuser__oxfax) }]*[{/if}]" type="tel" size="37" maxlength="128" name="invadr[oxuser__oxfax]" value="[{if isset( $invadr.oxuser__oxfax ) }][{ $invadr.oxuser__oxfax }][{else }][{ $oxcmp_user->oxuser__oxfax->value }][{/if}]">
